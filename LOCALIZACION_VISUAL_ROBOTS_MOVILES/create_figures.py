"""
THIS PROGRAM CONTAINS FUNCTIONS THAT CREATE FIGURES THAT GRAPHICALLY REPRESENT THE TEST RESULTS
Among these graphics, we can find:
- Maps with the network predictions
Test scripts will call these functions
"""
...
def display_coord_map(direc, mapVM, mapTest, k, ilum, env):
    ...
    plt.ylabel('y (m)')
    plt.xlabel('x (m)')
    plt.title(f'Environment: {env}, Illumination: {ilum}')
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(direc, "map_" + env + "_" + ilum + ".png"), dpi=400)
    plt.close()
